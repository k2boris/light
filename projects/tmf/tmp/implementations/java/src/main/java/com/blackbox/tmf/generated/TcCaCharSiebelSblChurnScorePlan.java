 ...
 */
public final class TcCaCharSiebelSblChurnScorePlan implements MappingPlan, ReverseMappingPlan {
    private static final Logger LOG = Logger.getLogger(TcCaCharSiebelSblChurnScorePlan.class.getName());

    @Override
    public String planId() {
        return "TC_CA_CHAR__Siebel__SBL_CHURN_SCORE";
    }
    ...
    public void execute(MappingContext context) throws Exception {
        ...
        try (PreparedStatement statement = context.sources().connection("SIEBEL_SYSTEM").prepareStatement("""
                select customer_id, churn_score, risk_band, model_version, scored_dt, top_driver from SBL_CHURN_SCORE where customer_id = ?
                """)) {
            statement.setString(1, customerId);
            try (ResultSet rs = statement.executeQuery()) {
                while (rs.next()) {
                    SourceRow row = new SourceRow(rs.getString("customer_id"), rs.getString("churn_score"), rs.getString("risk_band"), rs.getString("model_version"), rs.getString("scored_dt"), rs.getString("top_driver"));
    ...
    private static int insertRows(Connection target, String custAcctId, SourceRow row) throws SQLException {
        int rows = 0;
            if (!isBlank(row.churnScore())) {
                rows += insertCharacteristic(target, custAcctId, "SBL_CHURN_SCORE", "churnScore", row.churnScore());
            }
            if (!isBlank(row.riskBand())) {
                rows += insertCharacteristic(target, custAcctId, "SBL_CHURN_SCORE", "riskBand", row.riskBand());
            }
            if (!isBlank(row.modelVersion())) {
                rows += insertCharacteristic(target, custAcctId, "SBL_CHURN_SCORE", "modelVersion", row.modelVersion());
            }
            if (!isBlank(row.scoredDt())) {
                rows += insertCharacteristic(target, custAcctId, "SBL_CHURN_SCORE", "scoredDt", row.scoredDt());
            }
            if (!isBlank(row.topDriver())) {
                rows += insertCharacteristic(target, custAcctId, "SBL_CHURN_SCORE", "topDriver", row.topDriver());
            }
        return rows;
    }
    ...
    private record SourceRow(String customerId, String churnScore, String riskBand, String modelVersion, String scoredDt, String topDriver) {
    }
