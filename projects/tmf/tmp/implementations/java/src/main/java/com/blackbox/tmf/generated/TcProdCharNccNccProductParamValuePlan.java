...
public final class TcProdCharNccNccProductParamValuePlan implements MappingPlan, ReverseMappingPlan {
    private static final Logger LOG = Logger.getLogger(TcProdCharNccNccProductParamValuePlan.class.getName());

    @Override
    public String planId() { return "TC_PROD_CHAR__NCC__NCC_PRODUCT_PARAM_VALUE"; }

    @Override
    public void execute(MappingContext context) throws Exception {
        String customerId = context.instanceValue("customer_id");
        int rows = 0;
        try (PreparedStatement statement = context.sources().connection("NCC_SYSTEM").prepareStatement("select v.product_id, v.param_name, v.param_value from NCC_SUBSCRIPTION s join NCC_PRODUCT p on s.subscription_id = p.subscription_id join NCC_PRODUCT_PARAM_VALUE v on p.product_id = v.product_id where s.customer_id = ? and p.status = 'ACTIVE'")) {
